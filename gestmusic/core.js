...
async function sbFetch(path, method = 'GET', body = null, prefer = '') {
  ...
  const res = await fetch(`${SUPA_URL}/rest/v1/${path}`, options);
...
// UPSERT em massa — insere ou actualiza por id
function sbUpsert(table, records) {
  const arr = Array.isArray(records) ? records : [records];
  return sbFetch(table, 'POST', arr, 'resolution=merge-duplicates,return=minimal');
}
...
// Persiste UMA tabela: upsert locais + apaga os removidos
async function saveStore(storeName) {
  const arr = appData[storeName] || [];

  // Upsert todos os registos locais de uma vez
  if (arr.length > 0) {
    await sbUpsert(storeName, arr);
  }

  // Apagar remotos que já não existem localmente
  const remote    = await sbSelect(storeName).catch(() => []);
  const localIds  = new Set(arr.map(r => String(r.id)));
  const toDelete  = remote.filter(r => !localIds.has(String(r.id)));
  if (toDelete.length > 0) {
    await Promise.all(toDelete.map(r => sbDelete(storeName, r.id)));
  }
}

// Versão completa: persiste todos os dados (usado no import/setup)
async function saveData(data) {
  await Promise.all(TABLES.map(async t => {
    const arr = data[t] || [];
    if (arr.length > 0) await sbUpsert(t, arr);
  }));
}

// Wrapper chamado pelos módulos: persiste uma tabela
function persist(storeName) {
  if (window.location.protocol === 'file:') {
    notify('Abre via servidor (Live Server ou Netlify) para guardar dados.', 'error');
    return;
  }
  saveStore(storeName).catch(e => {
    ...
    if (msg.includes('row-level security') || msg.includes('policy')) {
      notify('Erro RLS: executa o SQL das politicas no Supabase.', 'error');
    } else if (msg.includes('unique') || msg.includes('duplicate')) {
      notify('Codigo de portal duplicado. Gera um novo codigo.', 'error');
    } else if (msg.includes('foreign key') || msg.includes('violates')) {
      notify('Erro de referencia. Guarda o artista antes do projeto.', 'error');
    } else if (msg.includes('API key') || msg.includes('401')) {
      notify('Chave Supabase invalida. Verifica o core.js.', 'error');
    } else {
      notify('Erro Supabase: ' + (msg.slice(0,80) || 'Verifica a ligacao.'), 'error');
    }
  });
}

// Teste de ligacao ao Supabase
...
// Ecrã de erro — protocolo file://
function buildFileProtocolError() {
  return `
  <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:var(--bg-deep);padding:20px">
    <div style="background:var(--bg-card);border:1px solid rgba(245,158,11,.4);border-radius:16px;padding:36px;max-width:500px;width:100%;text-align:center">
      <div style="font-size:48px;margin-bottom:16px">🚫</div>
      <div style="font-family:Bebas Neue;font-size:24px;letter-spacing:2px;color:var(--orange);margin-bottom:10px">ABERTO EM FILE://</div>
      <div style="font-size:14px;color:var(--text-secondary);margin-bottom:20px;line-height:1.7">
        O browser bloqueia ligações ao Supabase quando o ficheiro é aberto directamente.<br>
        <strong style="color:var(--gold)">Precisas de um servidor local ou do Netlify.</strong>
      </div>
      <div style="background:var(--bg-deep);border-radius:12px;padding:20px;text-align:left;font-size:13px;color:var(--text-secondary);line-height:2;margin-bottom:20px">
        <div style="color:var(--gold);font-weight:700;margin-bottom:8px">✅ OPÇÃO 1 — VS Code (mais rápido)</div>
        <div>1. Instala a extensão <strong style="color:var(--neon)">Live Server</strong></div>
        <div>2. Clica com botão direito em <code style="color:var(--gold)">admin.html</code></div>
        <div>3. Clica em <strong>Open with Live Server</strong></div>
        <div style="margin-top:12px;color:var(--gold);font-weight:700;margin-bottom:8px">✅ OPÇÃO 2 — Python</div>
        <div style="font-family:Space Mono;font-size:11px;background:rgba(0,0,0,.3);padding:8px 12px;border-radius:8px;margin-top:4px">python -m http.server 8080</div>
        <div style="margin-top:6px">Depois abre: <code style="color:var(--neon)">http://localhost:8080/admin.html</code></div>
        <div style="margin-top:12px;color:var(--gold);font-weight:700;margin-bottom:8px">✅ OPÇÃO 3 — Netlify (definitivo)</div>
        <div>Arrasta a pasta para <a href="https://netlify.com" target="_blank" style="color:var(--gold)">netlify.com</a> — ver GUIA_SUPABASE.md</div>
      </div>
...
async function bootApp() {
  ...
  // Verificar se está a correr em file:// (bloqueia ligação ao Supabase)
  if (window.location.protocol === 'file:') {
    document.getElementById('root').innerHTML = buildFileProtocolError();
    return;
  }
